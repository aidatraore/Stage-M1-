...
#       │
#       ├──→ primary_seq   : base dominante (A/C/G/T/N uniquement — jamais IUPAC)
#       │                    si N → base au signal le plus fort
#       │                    si égalité → on prend quand même la plus forte (on tranche)
#       │
...
############################################################
# 3) Trimming Q30 par fenêtre glissante
############################################################
def find_trim_bounds_window(qual, threshold=30, window=10):
    """
    Trimming par fenêtre glissante :
    avance tant que la qualité MOYENNE sur la fenêtre < threshold.
    Plus robuste que position par position pour Q30.
    """
    start = 0
    while start < len(qual) - window:
        if np.mean(qual[start:start + window]) >= threshold:
            break
        start += 1

    end = len(qual) - 1
    while end >= window:
        if np.mean(qual[end - window:end]) >= threshold:
            break
        end -= 1

    return start, end

start, end = find_trim_bounds_window(qual, threshold=30, window=10)
end = min(end, len(peak_locations) - 1)

if start >= end:
    raise ValueError(
        f"Qualité trop faible : aucune région Q30 trouvée. "
...
############################################################
# 4) Seuil min_height dynamique (10e percentile des pics)
############################################################
all_peak_max = [
...
min_height = int(np.percentile(all_peak_max, 10))
print(f"[INFO] Seuil min_height dynamique : {min_height}")
...
# 6) Fonctions
############################################################
def get_primary(a, c, g, t, min_height=100):
    """
    Base PRIMAIRE : signal le plus fort.
    → Toujours A/C/G/T ou N (jamais IUPAC)
    → N si signal trop faible
    """
...
def correct_N_primary(a, c, g, t, min_height=100):
    """
    Correction N dans primary : on tranche toujours sur le signal max.
    → Jamais IUPAC
    """
    return get_primary(a, c, g, t, min_height)
...
def pool_consensus(primary, secondary):
    ...
    - primary   : A/C/G/T/N (jamais IUPAC)
    - secondary : A/C/G/T ou code IUPAC ou None
    - N si primary == N
    - primary si secondary == None
    - IUPAC union sinon
    """
    if primary == "N":
        return "N"
...
for idx in range(start, end + 1):
    ...
    # PRIMARY : toujours A/C/G/T/N — jamais IUPAC
    if raw_base != "N":
        primary = raw_base
    else:
        primary = correct_N_primary(a, c, g, t, min_height=min_height)
    primary_seq += primary

    # SECONDARY : A/C/G/T ou IUPAC si N et égalité
    if raw_base != "N":
...
# 8) Vérifications
############################################################
assert set(primary_seq).issubset(set("ACGTN")), \
    f"ERREUR : codes IUPAC dans primary_seq : {set(primary_seq) - set('ACGTN')}"

assert set(secondary_seq).issubset(set("ACGTNRYSWKMBDHV")), \
...
print(f"[INFO] primary   : uniquement A/C/G/T/N ✓")
print(f"[INFO] secondary : peut contenir codes IUPAC aux positions N ✓")
...
with open(prefix + ".stats.txt", "w") as f:
    ...
    f.write(f"Codes IUPAC dans secondary  : {nb_iupac_secondary}\n")
    f.write(f"Codes IUPAC dans consensus  : {nb_iupac_consensus}\n\n")

    f.write("=== PARAMÈTRES UTILISÉS ===\n")
    f.write(f"Ordre canaux (FWO_1)    : {fwo}\n")
    f.write(f"Seuil trimming          : Q30 fenêtre glissante (window=10)\n")
    f.write(f"Seuil min_height        : {min_height}\n")
